...
import { supabase } from '../../supabase/client'; // Ajusta la ruta a tu cliente
...
export default function GiftCards() {
  ...
  useEffect(() => {
    checkSession();
    // Cargar estado inicial (Simulado desde localStorage por ahora, luego será DB)
    loadLocalState();
  }, []);
  ...
  const checkSession = async () => {
    ...
    // Aquí podrías cargar el perfil real
    // const { data } = await supabase.from('users').select('*').single();
    // setUser(data);
    setLoading(false);
  };

  // Carga temporal (Simulando Backend)
  const loadLocalState = () => {
    const saved = localStorage.getItem('offszn_giftcards_state');
    if (saved) {
      const parsed = JSON.parse(saved);
      setCurrency(parsed.currentCurrency || 'USD');
      setHasClaimedWelcome(parsed.hasClaimedWelcome || false);
      setLastSpinMonth(parsed.lastSpinMonth || null);
      setBalance(parsed.totalBalance || 0);
  ...
  const saveLocalState = (newState) => {
    const stateToSave = {
        ...
        hasClaimedWelcome: newState.hasClaimedWelcome ?? hasClaimedWelcome,
        lastSpinMonth: newState.lastSpinMonth ?? lastSpinMonth,
        totalBalance: newState.balance ?? balance,
        giftCards: newState.giftCards ?? giftCards
    };
    localStorage.setItem('offszn_giftcards_state', JSON.stringify(stateToSave));
  };
            ...
            <div className="flex flex-col items-center gap-4">
                <button 
                    ...
                    className={`px-12 py-4 rounded-xl font-bold text-lg flex items-center gap-3 transition-all ${
                        spinMessage.disabled || isSpinning
